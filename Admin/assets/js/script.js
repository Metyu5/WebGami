...
// --- Perbaikan di sini: Pastikan kapitalisasi 'Admin' sesuai dengan folder fisik Anda ---
function getPhysicalBaseUrl() {
    // Sesuaikan ini HANYA JIKA NAMA FOLDER 'WebGami' ATAU 'Admin' di Laragon Anda BEDA KAPITALISASI
    // Contoh: Jika di Laragon adalah C:\laragon\www\webgami\admin\ maka ubah menjadi '/webgami/admin/'
    // Berdasarkan screenshot Anda, 'WebGami/Admin' sudah benar.
    return window.location.origin + '/WebGami/Admin/';
}
...
async function handleFormSubmission(form) {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            let pageToReload = '../content/dashboard_content.php'; // Default fallback (dalam format data-page)

            if (currentPageContainer) {
                const pageIdentifier = currentPageContainer.dataset.currentPage;
                if (pageIdentifier === 'student') {
                    pageToReload = '../content/student.php';
                } else if (pageIdentifier === 'walikelas') {
...
async function handleDeleteStudent(idToDelete) {
    try {
        ...
        const currentPageContainer = mainContent.querySelector('[data-current-page]');
        let targetUrlForAjax = getPhysicalBaseUrl() + 'content/dashboard_content.php'; // Default target AJAX
        let pageToReloadAfterDelete = '../content/dashboard_content.php'; // Default pageUrl untuk loadContent

        if (currentPageContainer) {
            const pageIdentifier = currentPageContainer.dataset.currentPage;
            if (pageIdentifier === 'student') {
                targetUrlForAjax = getPhysicalBaseUrl() + 'content/student.php';
                pageToReloadAfterDelete = '../content/student.php';
...
document.addEventListener('submit', e => {
    ...
    if (e.target.id === 'searchForm') {
        ...
        const currentPageContainer = mainContent.querySelector('[data-current-page]');
        let pageToLoad = '../content/dashboard_content.php'; // Default

        if (currentPageContainer) {
            const pageIdentifier = currentPageContainer.dataset.currentPage;
            if (pageIdentifier === 'student') {
                pageToLoad = '../content/student.php';
            } else if (pageIdentifier === 'walikelas') {
                // *** PERBAIKAN: Pastikan ini selalu menunjuk ke path yang benar dari root admin ***
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let pageParamFromUrl = urlParams.get('page') || 'dashboard_content'; // Ubah default menjadi 'dashboard_content' karena itu nama filenya

    let initialPageUrlForContent = '../content/dashboard_content.php'; // Default format data-page
    ...
    if (activeLinkOnLoad) {
        ...
        const dashboardLink = document.querySelector('a[data-page="../content/dashboard_content.php"]');
        if (dashboardLink) {
