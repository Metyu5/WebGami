    ...
    return {
        ...
        async submitForm() {
            ...
            try {
                ...
                const response = await fetch('../content/student.php', {
                    method: 'POST',
                    body: formData
                });
                ...
                if (result.status === 'success') {
                    ...
                    if (this.isEditMode) {
                        // Jika berhasil edit, kosongkan search query dan reload halaman pertama
                        this.searchQuery = ''; // Hapus query pencarian
                        this.goToPage(1); // Muat ulang halaman pertama tanpa filter pencarian
                    } else { 
                        // Jika berhasil tambah siswa, tetap kosongkan search query dan ke halaman 1
                        this.searchQuery = ''; 
                        this.goToPage(1); 
                    }
                } else {
        ...
        goToPage(pageNumber) {
            ...
            const newUrl = `?content=student&page=${pageNumber}${searchParam}`;
        ...
        // Fungsi reloadPage ini tidak lagi digunakan langsung setelah edit/hapus
        // karena kita sekarang mengarahkan ke goToPage(1) dengan searchQuery direset.
        // Namun, jika ada bagian lain yang memanggilnya, ia akan tetap berfungsi.
        reloadPage() {
            if (typeof window.loadContent === 'function') {
